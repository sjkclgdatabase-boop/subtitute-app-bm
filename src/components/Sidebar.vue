<template>
  ...
  >
    ...
    <!-- 底部操作区：多语言切换（左右并排两列，使用 Languages 图标） + 退出登录 -->
    <div class="p-3 border-t border-slate-800 bg-slate-950/40 space-y-1.5">
      
      <!-- Language Switcher: 中文 + English (左右并排两列，无国旗，使用 Languages 图标) -->
      <div class="flex gap-1.5">

        <!-- 1. 切换到中文版按钮 -->
        <button
          @click="switchToLanguage('https://subtitute-app.vercel.app')"
          class="flex-1 min-w-0 flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-2xl text-xs font-bold text-indigo-400 hover:bg-indigo-500/10 hover:text-indigo-300 transition-colors cursor-pointer"
          title="中文"
        >
...
const navItems = [
  { name: 'GAMBARAN KESELURUHAN', path: '/', icon: LayoutDashboard },
  { name: 'PENGURUSAN CUTI GURU', path: '/leave-entry', icon: CalendarCheck2 },
  { name: 'PROFIL GURU', path: '/teachers', icon: UsersRound },
  { name: 'JADUAL WAKTU GURU', path: '/timetable', icon: CalendarDays },
  { name: 'JADUAL WAKTU KELAS', path: '/class-timetable', icon: Table2 },
  { name: 'JADUAL GURU GANTI', path: '/records', icon: ArrowLeftRight },
  { name: 'PUSAT LAPORAN MMI', path: '/statistics', icon: ChartNoAxesCombined },
  { name: 'REKOD GANGGUAN MMI', path: '/mmi-interruption', icon: TriangleAlert }, 
  { name: 'ANALISIK SUBJEK', path: '/subject-analysis', icon: ChartColumnBig },
  { name: 'TETAPAN SISTEM', path: '/settings', icon: Settings2 } 
]
