          ...
          <div class="p-8 bg-slate-50/50 space-y-6 overflow-y-auto">
            ...
            <div>
              <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-2">
                ...
                SENARAI CALON CADANGAN PINTAR (TOP 6)
              </h3>
            ...
            <div class="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-[10px] text-slate-500 font-medium leading-relaxed">
                  Jika ditandai, sistem akan mencipta rekod maya (tidak menjejaskan MMI) dan menambah jumlah kelas guru ini sebanyak +1 secara latar belakang.
                </p>
              </div>
            </div>
          </div>
...
const handleCellClick = async (
  ...
  try {
    ...
    toast.error(
      'Gagal memuatkan data jadual: ' + err.message
    )
...
const handleAutoAssignAll = async () => {
  ...
  if (pendingRequests.length === 0) {
    toast.success(
      'Tiada kelas yang perlu ditetapkan untuk sesi ini!'
    )
  ...
  try {
    ...
    toast.success(
      `Berjaya! Sebanyak ${successCount} kelas telah ditetapkan secara automatik.`
    )

    fetchData()
  } catch (err) {
    toast.error(
      'Ralat semasa penetapan automatik: ' +
        err.message
    )
  } finally {
...
const confirmBlankAssignment = async () => {
  ...
  if (
    ...
    return toast.error(
      'Sila pilih guru jika ingin mengira beban!'
    )
  }
...
const removeBlankAssignment = async () => {
  ...
  try {
    ...
    toast.success(
      'Berjaya dikosongkan dan beban dibatalkan!'
    )
  ...
  try {
    ...
    const drawCenteredText = (
      ...
      for (
        ...
        // If the text is too tall, reduce font size.
        if (
          totalHeight >
          innerHeight
        ) {
