<template>
  <div class="p-4 sm:p-8 mx-auto min-h-screen space-y-8 min-w-[1024px]">
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm ring-1 ring-slate-900/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:hidden">
      <div class="space-y-2 max-w-3xl">
        ...
        <p class="text-slate-500 text-xs sm:text-sm font-medium leading-relaxed">
          SOKONGAN PENGECAMAN KELAS BERGABUNG PINTAR, PAPARAN PELBAGAI GURU, PEMBAHAGIAN SEL, SERTA PENGABUNGAN WAKTU BERKEMBAR SECARA AUTOMATIK.
        </p>
      </div>
    ...
    <div v-if="!selectedClass && !isLoading" class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-900/5 p-16 text-center flex flex-col items-center justify-center print:hidden">
      ...
      <h3 class="text-base font-bold text-slate-900">SILA PILIH KELAS DI ATAS</h3>
      <p class="text-slate-500 mt-1 text-xs font-medium">SELESAI PILIH, PAPAR DAN CETAK JADUAL KELAS DI SINI</p>
    </div>
    ...
    <div v-if="selectedClass" class="bg-white p-4 sm:p-8 rounded-3xl shadow-sm ring-1 ring-slate-200 overflow-x-auto print:overflow-visible print:border-none print:shadow-none print:p-0 print:m-0 print-safe-font print:pt-6">
      ...
      <div class="min-w-[1050px] print:min-w-0 print:w-full mx-auto bg-white text-black relative space-y-3">
        ...
        <!-- 表头 -->
        <div class="flex items-center justify-between border-b-[3px] border-black pb-3">
        ...
        <!-- 课表 Table (保留 REHAT 灰底) -->
        <table class="w-full border-collapse border-[2px] border-black text-center table-fixed print:text-[11px]">
          <thead>
            <tr>
              <th class="border border-black w-16 sm:w-24 py-1"></th>
              <!-- 第 1-5 节 -->
              <th v-for="p in 5" :key="p" class="border border-black py-1">
              ...
              <!-- 动态渲染 REHAT 之后的节数 -->
              <th v-for="p in (totalPeriods - 5)" :key="p+5" class="border border-black py-1">
          ...
          <tbody>
            <tr v-for="(dayData, dIndex) in gridData" :key="dayData.day">
              <!-- 星期 -->
              <td class="border border-black font-bold text-sm sm:text-lg h-16 sm:h-20 text-black">
                {{ dayData.day }}
              </td>
              ...
              <template v-for="(cell, cIndex) in dayData.cells" :key="cIndex">
                ...
                <td 
                    ...
                    class="border border-black relative h-16 sm:h-20 p-0 overflow-hidden"
                    >
                    <!-- 空格子 -->
                    <div v-if="cell.lessons.length === 0" class="w-full h-full flex items-center justify-center text-slate-300 text-xs">
                    ...
                    <div v-else class="w-full h-full flex flex-col">
                        ...
                        :key="lIndex" 
                        class="flex-1 flex flex-col justify-center items-center relative py-1"
    ...
    <div v-if="isLoading" class="mt-12 text-center text-indigo-500 font-bold print:hidden">
      Sedang menganalisis data kelas dan jadual, sila tunggu sebentar...
    </div>
...
const fetchTimetable = async () => {
  ...
  try {
    ...
    console.error("Gagal memuatkan jadual:", error)
  } finally {
